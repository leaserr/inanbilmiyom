...
/*async function TagAlıncaİsimKontrol() { // Tag alınca tarama
  ...
  const members = [...guild.members.cache.filter(u => u.user.globalName && u.user.globalName.includes(conf.tag) && u.globalName.includes(conf.ikinciTag)).values()].splice(0, 10)
  for await (const member of members) {
  await member.setNickname(member.globalName.replace(conf.ikinciTag, conf.tag))
}
};

async function TagSalıncaİsimKontrol() { // Tag alınca tarama
  const guild = client.guilds.cache.get(ramalcim.GuildID)
  const members = [...guild.members.cache.filter(u => u.user.globalName && u.user.globalName.includes(conf.tag) && u.globalName.includes(conf.tag)).values()].splice(0, 10)
  for await (const member of members) {
  await member.setNickname(member.globalName.replace(conf.tag, conf.ikinciTag))
}
};*/
