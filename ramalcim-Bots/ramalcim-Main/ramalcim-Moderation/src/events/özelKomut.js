...
const özelPerms = require("../../../../../Global/schemas/talentPerms");
...
module.exports = async (message) => {
    ...
    yazilanKomut = yazilanKomut.slice(ramalcim.Main.prefix.some(x => x.length));
    ...
    if (message.member.roles.cache.some(rol => komut.YetkiliRol.includes(rol.id)) || message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
        ...
        if (!komut.verilecekRol.some(rol => üye.roles.cache.has(rol))) {
            ...
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${ramal_Yes} Başarılı şekilde ${üye} kişisine ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü verdim.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
            ...
            message.react(ramal_Yes)
            message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${ramal_Yes} Başarılı şekilde ${üye} kişisinden ${komut.verilecekRol.map(x => `<@&${x}>`)} rolünü aldım.`)] }).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
