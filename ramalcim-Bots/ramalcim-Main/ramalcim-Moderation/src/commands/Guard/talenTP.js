...
module.exports = {
  ...
  run: async (client, message, args) => {  
    ...
    if (args[0] === "oluştur" || args[0] === "ekle") {
        ...
        message.channel.send({ embeds: [new EmbedBuilder().setDescription(`${ramal_Yes} \`${komutAd}\` adlı komut başarılı bir şekilde oluşturuldu.\n\nKomut İzni Olan Rol(ler): ${yetkiliRol}\nVerilecek Rol(ler): ${roller}\nKomut Log Kanal: ${kanalcık}`)] });
        ...
        await message.channel.send({ content: `${ramal_Yes} \`${args[1]}\` isimli komut başarılı bir şekilde silindi.`}).then((e) => setTimeout(() => { e.delete(); }, 5000)).catch(err => {});
    }
  }
};