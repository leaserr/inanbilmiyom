...
const { red, ramal_Yes, Mute, revusome, kirmiziok, Cezaa, Revuu } = require("../../../../src/configs/emojis.json")
...
if (ramalcim.Main.dmMessages) member.send({ content :`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle **kalıcı olarak** banlandınız!`}).catch(() => {});
...
msg.edit({ content :`${ramal_Yes} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})
...
if(button.customId === "üç") {
  ...
  return }
  if (ramalcim.Main.banlimit > 0 && banLimit.has(message.author.id) && banLimit.get(message.author.id) == ramalcim.Main.banlimit) return message.channel.send({ content:"Saatlik ban sınırına ulaştın!", ephemeral: true })

  if (ramalcim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **Reklam** sebebiyle banlandınız!`}).catch(() => {});
  ...
  msg.edit({ content :`${ramal_Yes} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`Reklam\` nedeniyle banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})
