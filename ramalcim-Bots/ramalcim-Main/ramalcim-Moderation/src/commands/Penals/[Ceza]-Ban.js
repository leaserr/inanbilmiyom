...
const bansistem = require("../../../../../../Global/schemas/system")
const banLimit = new Map();
moment.locale("tr");
const conf = require("../../../../src/configs/sunucuayar.json")
const ramalcim = require("../../../../../../Global/BotSettings/Settings")
const { red, ramal_Yes, Cezaa, Revuu, kirmiziok } = require("../../../../src/configs/emojis.json")
const sevismekoh = require("../../../../../../Global/schemas/serverSetting")
module.exports = {
  conf: {
    aliases: ["ban","yargı"],
    name: "ban",
    help: "ban <favel/ID> <Sebep>",
    category: "cezalandırma",
  },

  run: async (client, message, args, embed) => {

    const bankilitdata = await bansistem.findOne({ guildID: message.guild.id })
    if (bankilitdata && bankilitdata.bankilit === true) {
    return message.reply({ embeds: [embed.setDescription(`
  Sunucu Ban Sistemi Bir Yönetici Tarafından Kapatılmıştır.
  
  Ban Sistemi Açılana Kadar Ban Yapamazsınız Lütfen Açılana Kadar Bekleyiniz
  `)]})
  }
  let db = await sevismekoh.findOne({ guild: message.guild.id });
