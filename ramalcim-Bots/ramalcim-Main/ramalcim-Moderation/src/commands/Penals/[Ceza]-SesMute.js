...
module.exports = {
  conf: {
    ...
    name: "vmute",
    help: "vmute <favel/ID> <Süre> <Sebep>",
  ...
  run: async (client, message, args, embed) => {
    ...
    return }
    if (ramalcim.Main.voicemutelimit > 0 && vmuteLimit.has(message.author.id) && vmuteLimit.get(message.author.id) == ramalcim.Main.voicemutelimit) 
    {
    message.react(red)
    ...
    if (duration) {
      ...
      if(msg) msg.delete();
      await message.channel.send({ content:`${ramal_Yes} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **sesli kanallarda** susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(ramal_Yes)
      if (ramalcim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar **sesli kanallarda** susturuldunuz.`}).catch(() => {});
      ...
      if(!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !conf.sahipRolu.some(x => message.member.roles.cache.has(x))) {
        if (ramalcim.Main.voicemutelimit > 0) {
    ...
    await interaction.deferUpdate();
    if(msg) msg.delete();
    interaction.followUp({ content: `${ramal_Yes} Sesli Susturma işlemi başarıyla iptal edildi.`, ephemeral: true });
    }
    })
    }
      },
