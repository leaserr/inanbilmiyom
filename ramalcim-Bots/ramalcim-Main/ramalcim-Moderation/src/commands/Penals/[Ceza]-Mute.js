...
const { red, ramal_Yes, Mute, revusome, kirmiziok } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client,message, args, embed) => {
    ...
    if (ramalcim.Main.chatmutelimit > 0 && muteLimit.has(message.author.id) && muteLimit.get(message.author.id) == ramalcim.Main.chatmutelimit) 
    {
    message.react(red)
    ...
    if (duration) {
      ...
      if(msg) msg.delete();
      await message.channel.send({ content:`${ramal_Yes} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(ramal_Yes)
      if (ramalcim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar susturuldunuz.`}).catch(() => {});
      ...
      if(!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !conf.sahipRolu.some(x => message.member.roles.cache.has(x))) {
        if (ramalcim.Main.chatmutelimit > 0) {
    ...
    const penal = await client.penalize(message.guild.id, member.user.id, "CHAT-MUTE", true, message.author.id, reason, true, Date.now() + duration);
    if(msg) msg.delete();
    interaction.followUp({ content:`${ramal_Yes} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
    if (ramalcim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, <t:${Math.floor((Date.now() + duration) / 1000)}:R>'ya kadar susturuldunuz.`}).catch(() => {});
    ...
    await interaction.deferUpdate();
    if(msg) msg.delete();
    interaction.followUp({ content: `${ramal_Yes} Susturma işlemi başarıyla iptal edildi.`, ephemeral: true });
    }
    })
    }
      },
