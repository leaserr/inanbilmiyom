...
const {red, ramal_Yes, Revuu} = require("../../../../src/configs/emojis.json")
const ayar = require("../../../../../../Global/Settings/Bot-Commands")
module.exports = {
  conf: {
    aliases: ["unmute","uncmute"],
    name: "unmute",
    help: "unmute <favel/ID>",
  ...
  collector.on("collect", async (button) => {
    ...
    if (button.customId === "mute") {
      ...
      if (ramalcim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından susturmanız kaldırıldı!`}).catch(() => {});
      ...
      await msg.edit({ embeds: [ramal], components: [row] });
    ...
    if (button.customId === "vmute") {
      ...
      if (ramalcim.Main.dmMessages) member.send({ content:`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından **sesli kanallarda** olan susturmanız kaldırıldı!`}).catch(() => {});
