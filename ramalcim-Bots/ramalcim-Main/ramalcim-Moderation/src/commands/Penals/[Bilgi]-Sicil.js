...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 }).exec(async (err, res) => {
        ...
        let ramalcim = table(xd.slice(0, 15), config)
        let ramal = table(xd2, config)

        let data = await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 });
        if (data.length === 0) return message.channel.send({ content:`${ramal_Yes} ${member.toString()} üyesinin sicili temiz!`}).then((e) => setTimeout(() => { e.delete(); }, 5000));
    ...
    if (button.customId === "dosya") {
      ...
      button.reply({ content: `${member} kullanıcısının toplamda **${cezaData ? cezaData.ceza.length : 0}** cezası  bulunmaktadır ve aşağıdaki belgede yazmaktadır.`, files: [{ attachment: Buffer.from(ramal), name: `${member.user.username}_sicil.txt` }], components: [], ephemeral: true }); 
