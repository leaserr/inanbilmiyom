...
let kanallar = ["tweet", "tweet-chat", "notebook"]
if (!kanallar.some((x) => message.channel.name.toLowerCase().includes(x))) return message.reply({content: `${message.guild.emojis.cache.find(x => x.name == emojis.no)} tweet kanallarında kullanabilirsiniz.`}).sil(15)
