...
const applyText = (canvas, text) => {
      ...
      let background = await Canvas.loadImage(client.guilds.cache.get(ramalcim.GuildID).bannerURL({dynamic: true, extension: "png"}) ? client.guilds.cache.get(ramalcim.GuildID).bannerURL({dynamic: true, extension: "png"}) + `?size=4096` : "https://cdn.discordapp.com/attachments/938786568175513660/970470122806472785/sex.png");
...
${shipdurum}`)
.setImage("attachment://ramal.png")
message.channel.send({ content: `**[ \`${member.user.tag}\` ]**`, embeds: [ramal], files: [attachment] })
} else if (random <= 40) {
