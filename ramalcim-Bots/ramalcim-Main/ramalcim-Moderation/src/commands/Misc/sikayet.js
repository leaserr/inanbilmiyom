...
module.exports = {
    ...
    if(!uye) return message.channel.send("\`.sikayet <@favel/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\`")
    ...
    let tür = args[1];
    if(tür !== "chat" && tür !== "sesli" && tür !== "taciz" && tür !== "dm") return message.channel.send("\`.sikayet <@favel/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\`")
    let şikayetbilgi = args.slice(2).join(' ');
    if(!şikayetbilgi) return message.channel.send("\`.sikayet <@favel/ID> <chat-sesli-taciz-dm> <Şikayet Açıklaması>\`")
