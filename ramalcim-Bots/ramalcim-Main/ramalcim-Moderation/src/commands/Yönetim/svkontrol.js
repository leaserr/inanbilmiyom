...
await button.reply({ embeds: [ramal], components : [], ephemeral: true });
}
...
Sunucumuza **1 gün** içerisinde ${rakam(message.guild.members.cache.filter(ramal => (new Date().getTime() - ramal.joinedTimestamp) < 1000 * 60 * 60 * 24).size)} kullanıcı giriş yapmış.
