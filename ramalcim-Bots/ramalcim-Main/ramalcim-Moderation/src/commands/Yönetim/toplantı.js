...
const { star, red, ramal_Yes } = require("../../../../src/configs/emojis.json");
...
embed.addFields({ name: `${star} Son Toplantı Bilgisi`, value: `
> Toplantıya **\`${MeetingData.Joining.length}\`** yetkili katılmış ${ramal_Yes}
...
**${guild.name}** sunucusunda şuan da toplantı durumu: **${meetingStatus ? "Aktif" : "Devre-dışı"}** ${meetingStatus ? ramal_Yes : red}`)
        ...
        collector.on('collect', async (i) => {
          if(i.customId == "toplantıAç") {
              let meetingStatus = MeetingData.Toplantı || false
              if(meetingStatus) {
                await meetings.updateOne({guildID: ramalcim.GuildID}, {$set: {Toplantı: false}}, {upsert: true})
...
${star} **Bitirilen Toplantı Bilgisi**
> Bu toplantıda katılması beklenen **(\`${KatilimData.Leaving.length + KatilimData.Joining.length} yetkili\`)**
> Bu toplantıda katılım sağlayan: **(\`${KatilimData.Joining.length} yetkili\`)** ${ramal_Yes}
...
> Bu toplantı  **\`${meetingTime(KatilimData.endDate - KatilimData.Date)}\`** sürmüş.`)], components: []})
                ...
                let datax = await meetings.findOne({ guildID: ramalcim.GuildID })
                if(!datax) {await meetings.updateOne({guildID: ramalcim.GuildID}, {$set: {Toplantı: true}}, {upsert: true})}
                await meetings.updateOne({guildID: message.guild.id}, {$set: {
                  "Date": Date.now(),
                  "userID": i.user.id,
                ...
                let datax2 = await meetings.findOne({ guildID: ramalcim.GuildID })
                if (datax2) {
