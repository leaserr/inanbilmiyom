...
module.exports = {
  ...
  if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) return message.react(red)
    ...
    if (sec) {
      if(!client.guilds.cache.get(ramalcim.GuildID).roles.cache.get(sec)) {
      ...
      let ramal = message.guild.members.cache.filter(yetkili => yetkili.roles.cache.has(sec)).filter(yetkilises => !yetkilises.voice.channel  && yetkilises.presence && yetkilises.presence.status != "offline")
      ...
      let ramal = message.guild.members.cache.filter(yetkili => yetkili.roles.cache.has(conf.staffs[0])).filter(yetkilises => !yetkilises.voice.channel  && yetkilises.presence && yetkilises.presence.status != "offline")
