...
const Custom = require("../../../../../../Global/schemas/menü");
const ramalcim = require("../../../../../../Global/BotSettings/Settings")
const { red, ramal_Yes, star } = require("../../../../src/configs/emojis.json")
...
\` ❯ \` Düzenleme işlemi yaparken tekrardan aşağıda ki düğmeye basarak, düzenlenmesini istediğiniz rol seçim menüsü ismini girerek tekrardan ayarlarını güncelleyebilirsiniz.`)], components: comp}).then(async (msg) => {
                    ...
                    collector.on("collect", async (i) => {
                        if(i.customId == "sil") {
                            ...
                            i.reply({content: `${ramal_Yes} Başarıyla **${i.values}** isimli rol seçim menüsü silindi.`, ephemeral: true})
                        ...
                        if(i.customId == "kur") {
                            ...
                            if(kurulcak) {
                                ...
                                message.channel.send({content: `${kurulcak.Text}`, components: [listMenu]}).then(async (oluşturuldu) => {
                                    ...
                                    collector.on('collect', async (i) => {
                                        const member = await client.guilds.cache.get(ramalcim.GuildID).members.fetch(i.user.id)
                                        if (!member) return;
                                ...
                                i.reply({content: `${ramal_Yes} Başarıyla **${kurulcak.Name}** isimli rol seçim menüsü kuruldu.`, ephemeral: true})
                                msg.delete().catch(err => {})
                                message.react(ramal_Yes)
                            }
                        }
...
Yeni oluşturulmakta olan rol seçim menünüze bir isim belirleyin.`)]}).then(async (isimbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        ...
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return isimbelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`${red} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
.then(async (açıklamabelirleme) => { 
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return açıklamabelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`${red} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
Yeni oluşturulmakta olan rol seçim menünüzde listelenecek rolleri belirtin.`).setFooter({ text:`En az 3 tane, en fazla 25 tane rol ekleyebilirsiniz.`})]}).then(async (rolbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
           return rolbelirleme.edit({content: null, embeds: [new EmbedBuilder().setDescription(`${red} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
${ramal_Yes} Başarıyla **${rolSeçim.Name}** isimli rol seçim menüsü \`${moment(Date.now()).format("LLL")}\` tarihinde oluşturuldu.`)]}).then(async (oluşturuldu) => {
