...
const inviterData = await inviterSchema.findOne({ guildID: ramalcim.GuildID, userID: interaction.user.id });
const total = inviterData ? inviterData.total : 0;
const regular = inviterData ? inviterData.regular : 0;
const bonus = inviterData ? inviterData.bonus : 0;
const leave = inviterData ? inviterData.leave : 0;
const fake = inviterData ? inviterData.fake : 0;
const invMember = await inviteMemberSchema.find({ guildID: ramalcim.GuildID, inviter: interaction.user.id });
...
const data = await nameData.findOne({ guildID: ramalcim.GuildID, userID: member.id });

////////////////////////////////////////////////////////////////////////////////////////////

const messageData = await messageUser.findOne({ guildID: ramalcim.GuildID, userID: interaction.user.id });
const voiceData = await voiceUser.findOne({ guildID: ramalcim.GuildID, userID: interaction.user.id });
...
const category = async (parentsArray) => {
  const data = await voiceUserParent.find({ guildID: ramalcim.GuildID, userID: member.id });
