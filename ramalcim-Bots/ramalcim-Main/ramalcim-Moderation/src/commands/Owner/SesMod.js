...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    const data = await Database.findOne({ guildID: ramalcim.GuildID });
...
collector.on("collect", async (menu) => {
  await menu.deferUpdate();
    if(menu.values[0] === "mod") {
      ...
      await Database.findOneAndUpdate({ guildID: ramalcim.GuildID }, {SesMod: hosgeldin}, { upsert: true })
    ...
    if(menu.values[0] === "mod3") {
      ...
      await Database.findOneAndUpdate({ guildID: ramalcim.GuildID }, {SesMod: konser}, { upsert: true })
    ...
    if(menu.values[0] === "mod4") {
      ...
      await Database.findOneAndUpdate({ guildID: ramalcim.GuildID }, {SesMod: turnuva}, { upsert: true })
    ...
    if(menu.values[0] === "mod5") {
      ...
      await Database.findOneAndUpdate({ guildID: ramalcim.GuildID }, {SesMod: toplantı}, { upsert: true })
    ...
    if(menu.values[0] === "mod6") {
      ...
      await Database.findOneAndUpdate({ guildID: ramalcim.GuildID }, {SesMod: cekilis}, { upsert: true })
