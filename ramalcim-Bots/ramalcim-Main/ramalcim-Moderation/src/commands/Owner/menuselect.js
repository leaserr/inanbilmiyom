...
const { keasynitro, keasynetflix, keasyspotify, keasyexxen, keasyblutv, keasyyoutube, star } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client, message, args, durum, kanal) => {
    ...
    \` ⦁ \` Eğer \`@Çekiliş Katılımcısı\` Rolünü alırsanız sunucumuzda sıkça vereceğimiz ${keasynitro} , ${keasyspotify} , ${keasyyoutube} , ${keasynetflix} , ${keasyexxen} , ${keasyblutv} ve daha nice ödüllerin bulunduğu çekilişlerden haberdar olabilirsiniz. 
                    ...
                    "components": [{
                        "type": 1, "components": [{
                            "type": 3, "custom_id": "renk", "options": [
                              ...
                              { "label": "Siyah", "description": "siyah rengine sahip olmak için tıkla!", "value": "siyah", "emoji": { "name": "🕷️" }, },
                    ...
                    "components": [{
                        "type": 1, "components": [{
                            "type": 3, "custom_id": "iliski", "options": [
                              ...
                              { "label": "Sevgili Yapmıyorum", "value": "yapmiyorum", "emoji": { "id": "1177329875149926400" }, },
                                { "label": "Rol İstemiyorum", "value": "rolsil", "emoji": { "name": "🗑️" }, }
    ...
    **NOT:** \`Kayıtlı, kayıtsız olarak hepiniz bu kanalı görebilmektesiniz. Sunucumuz da everyone veya here atılmayacağından dolayı kesinlikle rollerinizi almayı unutmayın.\``,
    ...
    const member = await client.guilds.cache.get(ramalcim.GuildID).members.fetch(interaction.member.user.id)
    if (!member) return;
    
    const etkinlik = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.etkinlik))
    const cekilis = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.cekilis))
    ...
    const koç = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.koç))
    const boğa = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.boğa))
    const ikizler = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.ikizler))
    const yengeç = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.yengeç))
    const aslan = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.aslan))
    const başak = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.başak))
    const terazi = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.terazi))
    const akrep = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.akrep))
    const yay = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.yay))
    const oğlak = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.oğlak))
    const kova = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.kova))
    const balık = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Burçlar.balık))
    ...
    const csgo = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.csgo))
    const lol = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.lol))
    const valorant = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.valorant))
    const gta5 = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.gta5))
    const pubg = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.pubg))
    const fortnite = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Oyunlar.fortnite))
        ...
        if (interaction.customId === "games") {
          ...
          const kirmizi = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.kirmizi))
          const turuncu = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.turuncu))
          const mor = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.mor))
          const pembe = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.pembe))
          const yesil = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.yesil))
          const mavi = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.mavi))
          const sarı = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.sarı))
          const siyah = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.siyah))
          const beyaz = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.Renkler.beyaz))
    ...
    if (interaction.customId === "renk") {
      ...
      let renkroller = [kirmizi, turuncu, mor, pembe, yesil, mavi, sarı, siyah, beyaz]
      if (!member.roles.cache.has(conf.ekipRolu) && !member.roles.cache.has(conf.boosterRolu) && !member.permissions.has(PermissionsBitField.Flags.Administrator)) {
          ...
          const sevgili = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.İlişkiler.couple))
          const yalnız = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.İlişkiler.alone))
          const yapmiyorum = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(ayar.İlişkiler.yapmiyorum))
    
    if (interaction.customId === "iliski") {
      let ilişki = new Map([
          ["couple", sevgili],
          ["alone", yalnız],
          ["yapmiyorum", yapmiyorum],
        ])
        let role = ilişki.get(interaction.values[0])
        let iliskiroller = [sevgili, yalnız, yapmiyorum]
