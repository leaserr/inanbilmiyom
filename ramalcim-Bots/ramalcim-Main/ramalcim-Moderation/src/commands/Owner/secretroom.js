...
const { red, ramal_Yes, secret1, secret2, secret3, secret4, secret5, secret6, secret7, secret8, secret9, secret10 } = require("../../../../src/configs/emojis.json");
const datas  = require("../../../../../../Global/schemas/secretRoom");
const datas2  = require("../../../../../../Global/schemas/secretRoomStat");
...
client.on('interactionCreate', async interaction => {
    if(interaction.customId === "create") {
        ...
        const xd = await client.guilds.cache.get(ramalcim.GuildID).channels.cache.get(data?.ID)
...
const Channel = Guild?.channels.cache.get(room?.ID);
    ...
    return }
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(ramalcim.GuildID, { Connect: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(ramalcim.GuildID, { Connect: true });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(ramalcim.GuildID, { ViewChannel: false });
        ...
        await interaction.reply({ content: `Kanal sahibi olmadığınız için bu butonu kullanamazsınız.`, ephemeral: true })
        return }    
        await Guild?.channels?.cache.get(room?.ID)?.permissionOverwrites.edit(ramalcim.GuildID, { ViewChannel: true });
        ...
        let msg = await interaction.channel.send({ content: `<@${rooms.Owner}>`, embeds: [proramal], components: [pro] })
        ...
        if (button.customId === "onays") {
            ...
            .setAuthor({ name: client.guilds.cache.get(ramalcim.GuildID).name, iconURL: client.guilds.cache.get(ramalcim.GuildID).iconURL({ dynamic: true, size: 2048 })})
    ...
    collector.on('end', i => {
       msg.delete();
        ...
        await interaction.reply({ content: `Lütfen kanalınıza girmesini istediğiniz üye(leri) etiketleyiniz. \`Örn: @ramal @ramalcim\``, ephemeral: true })
       ...
       let yarak = await interaction.channel.send({ content: `Odaya erişimini kaldırmak istediğiniz üyeleri listeden seçiniz.`, components: [row]});
            ...
            yarak.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000 }).then((x) => {
                x.values.map(async (v) => {
                    ...
                    let memb = client.guilds.cache.get(ramalcim.GuildID).members.cache.get(v)
                    if (memb && memb.voice && memb.voice.channel) {
                        memb.voice.disconnect();
                      }
                });
                if (yarak) yarak.delete();
                interaction.channel.send({ content: `${ramal_Yes} Başarılı bir şekilde <@${x.values}> adlı üyenin odanıza erişim izni silindi.` });
                });
        ...
        await interaction.reply({ content: `Kanal sahipliğini aktarmak istediğiniz üyeyi etiketleyiniz. \`Örn: @ramal\``, ephemeral: true })
        interaction.channel.awaitMessages({ filter, max: 1 }).then(async function (collected) {
...
client.on("ready", async () => {
    const sunucu = client.guilds.cache.get(ramalcim.GuildID);
