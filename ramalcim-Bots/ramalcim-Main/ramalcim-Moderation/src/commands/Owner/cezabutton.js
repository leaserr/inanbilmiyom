...
const data = require("../../../../../../Global/schemas/penals");
...
client.on('interactionCreate', async interaction => {
  ...
  const cezapuanData = await cezapuan.findOne({ guildID: ramalcim.GuildID, userID: member.id });
 ...
 Toplamda \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanı\` ve (Toplam **${cezaData ? cezaData.ceza.length : 0}** Ceza) olarak gözükmekte.`, ephemeral: true
  ...
  let data = await penals.find({ guildID: ramalcim.GuildID, userID: member.id, active: true }).sort({ date: -1 });
  ...
  let datas = await penals.find({ guildID: ramalcim.GuildID, userID: member.id, active: true }).sort({ date: -1 });
