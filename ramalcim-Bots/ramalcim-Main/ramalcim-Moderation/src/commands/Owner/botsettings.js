...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    const row2 = new Discord.ActionRowBuilder().addComponents(
        new Discord.ButtonBuilder().setCustomId("allres").setLabel("Tüm Botları Yeniden Başlat").setEmoji("1075898389931249785").setStyle(Discord.ButtonStyle.Danger),
    );
    ...
    collector.on("collect", async (menu) => {
            ...
            await message.channel.send({ embeds: [embed.setDescription(`${botclient.user} isimli bot üzerinde hangi işlemi yapmak istersin?`)], components: [newrow] }).then(async (msj) => {
                ...
                col.on("collect", async (button) => {
                    ...
                    if (button.customId === "botupdateavatar") {
                        if (msj) msj.edit({ embeds: [embed.setDescription(`${ramal_Yes} ${botclient.user} isimli botun yeni profil resmini yükleyin veya bağlantısını girin. İşleminizi \`60 saniye\` içinde tamamlamazsanız otomatik olarak iptal edilecektir. İşlemi iptal etmek için \`iptal\` yazabilirsin.`)], components: [] });
                        ...
                        coll.on("collect", async (msg) => {
                            ...
                            botclient.user.setAvatar(avatar).then(() => {
                                ...
                                if (msj) msj.delete().catch(() => { });
                                message.channel.send({ embeds: [embed.setDescription(`${ramal_Yes} ${botclient.user} isimli botun profil resmi başarıyla güncellendi.`).setThumbnail(botclient.user.avatarURL())] }).then((s) => message.react(ramal_Yes) && setTimeout(() => { if (s) s.delete(); }, 20000));
                        ...
                        if (msj) msj.edit({ embeds: [embed.setDescription(`${ramal_Yes} ${botclient.user} isimli botun yeni ismini girin. İşleminizi \`60 saniye\` içinde tamamlamazsanız otomatik olarak iptal edilecektir. İşlemi iptal etmek için \`iptal\` yazabilirsin.`)], components: [] });
                        ...
                        coll.on("collect", async (msg) => {
                            ...
                            botclient.user.setUsername(isim).then(() => {
                                if (bekle) bekle.delete().catch(() => { });
                                if (msj) msj.delete().catch(() => { });
                                message.channel.send({ embeds: [embed.setDescription(`${ramal_Yes} ${botclient.user} isimli botun ismi başarıyla güncellendi.`).addFields({ name: "İsim", value: `\`${eskinick}\` --> \`${botclient.user.username}\``, inline: false })] }).then((s) => message.react(ramal_Yes) && setTimeout(() => { if(s) s.delete(); }, 20000));
                                const log = client.channels.cache.find(x => x.name == "bot_log")
        ...
        if (menu.customId == "allres") {
            ...
            if (mesaj) mesaj.delete().catch(() => { });
            if(ramalcim.Destek.Active) {
                children.exec(`pm2 restart ${ramalcim.GuildName}_Destek`);
            }
