...
const { ramalnitro, ramalnetflix, ramalspotify, ramalexxen, ramalblutv} = require("../../../../src/configs/emojis.json")
...
const etkinlik = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(conf.etkinlik))
const cekilis = await client.guilds.cache.get(ramalcim.GuildID).roles.cache.find(x => x.name.includes(conf.cekilis))
