...
let collector = await msg.createMessageComponentCollector({ filter, time: 60000 })
    ...
    collector.on("collect", async (button) => {
      ...
      .setAuthor({ name: member.displayName, iconURL: member.user.avatarURL({ dynamic: true })})
      .setTimestamp()
      .setDescription(`${message.author}, ${member.toString()} Adlı kullanıcı senin yetki aldırma isteğini onayladı. ${ramal_Yes}`)
      ...
      client.channels.cache.find(x => x.name == "yetki_log").wsend({ content:`${message.author} \`(${message.author.id})\` kişisi ${member} \`(${member.id})\` kişisini yetkiye aldı! ${ramal_Yes}`})
