...
let collector = await msg.createMessageComponentCollector({ filter, time: 60000 })
    ...
    collector.on("collect", async (button) => {
      ...
      .setAuthor({ name: member.displayName, iconURL: member.user.avatarURL({ dynamic: true })})
      .setTimestamp()
      .setDescription(`${message.author}, ${member.toString()} Adlı kullanıcı senin taglı aldırma isteğini onayladı. ${ramal_Yes}`)
      
      message.member.updateTask(message.guild.id, "tagli", 1); 
      await coin.findOneAndUpdate({ guildID: member.guild.id, userID: message.author.id }, { $inc: { coin: ramalcim.Main.taggedCoin } }, { upsert: true });
