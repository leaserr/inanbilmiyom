...
const { red, ramal_Yes, Revuu, kirmiziok } = require("../../../../src/configs/emojis.json")
...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.reply({ content: `Kanıt için ekran görüntüsünü atınız. 2 dakika süreniz var, atılmazsa işlem iptal edilecek.`})
    .then(() => {
      message.channel.awaitMessages({
        ...
        message.reply({ content :`${ramal_Yes} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``});
        if (ramalcim.Main.dmMessages) member.send({ content :`**${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle jaillendiniz.`}).catch(() => {});
