...
run: async (client, message, args, prefix) => {
  ...
  if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator) && !kanallar.includes(message.channel.name)) return message.reply({ content: `${kanallar.map(x => `${client.channels.cache.find(chan => chan.name == x)}`)} kanallarında kullanabilirsiniz.`}).then((e) => setTimeout(() => { e.delete(); }, 10000)); 
    ...
    if (sec) {
      if(!client.guilds.cache.get(ramalcim.GuildID).roles.cache.get(sec.id)) {
    ...
    return data.length > 0 ? data.map((x, index) => `${index + 1} ${client.guilds.cache.get(ramalcim.GuildID).members.cache.get(x.userID).user.tag} : ${Number(x[type]).toLocaleString()} mesaj`) .join("\n") : "Veri bulunmuyor!";
    }
    const voiceData = async (type) => {
    let data = await voiceUser.find({ guildID: message.guild.id }).sort({ topStat: -1 });
    data = data.filter((x) => message.guild.members.cache.has(x.userID) && message.guild.members.cache.get(x.userID).roles.cache.has(sec.id));
    return data.length > 0 ? data.map((x, index) => `${index + 1} ${client.guilds.cache.get(ramalcim.GuildID).members.cache.get(x.userID).user.tag} : ${moment.duration(x[type]).format("H [saat], m [dakika] s [saniye]")}`) .join("\n") : "Veri bulunmuyor!";
    }
...
➥ ${client.guilds.cache.get(ramalcim.GuildID).roles.cache.get(sec.id).name} Rolündeki Üyeler ve Ses Bilgileri:

${await voiceData("topStat")}

────────────────────────────────────

➥ ${client.guilds.cache.get(ramalcim.GuildID).roles.cache.get(sec.id).name} Rolündeki Üyeler ve Mesaj Bilgileri:

${await messageData("topStat")}

────────────────────────────────────

➥ ${client.guilds.cache.get(ramalcim.GuildID).roles.cache.get(sec.id).name} Rolündeki Üyeler ve Yayın Bilgileri:

${await yayındata("topStat")}

────────────────────────────────────

➥ ${client.guilds.cache.get(ramalcim.GuildID).roles.cache.get(sec.id).name} Rolündeki Üyeler ve Kamera Bilgileri:

${await kameradata("topStat")}

────────────────────────────────────`

message.channel.send({ content:`${client.guilds.cache.get(ramalcim.GuildID).roles.cache.get(sec.id).toString()} rolüne sahip üyelerin verileri;`, files: [{ attachment: Buffer.from(text), name: "rolstat.js" }] });
