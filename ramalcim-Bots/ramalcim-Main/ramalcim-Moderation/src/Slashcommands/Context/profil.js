...
module.exports = {
  ...
  async execute(interaction, client) {
	...
	.addComponents(
		...
		let nameData = await isimler.findOne({ guildID: ramalcim.GuildID, userID: üye.id });
		let registerData = await register.findOne({ guildID: ramalcim.GuildID, userID: üye.id });
	  
				 const roles = client.guilds.cache.get(ramalcim.GuildID).members.cache.get(interaction.targetId).roles.cache.filter(role => role.id !== ramalcim.GuildID).sort((a, b) => b.position - a.position).map(role => `<@&${role.id}>`);
				  const rolleri = []
				  if (roles.length > 6) {
	...
	{name: `❯ Sunucu Bilgisi`,value:`
	  ...
	  \` • \` Katılım Tarihi: <t:${Math.floor(client.guilds.cache.get(ramalcim.GuildID).members.cache.get(interaction.targetId).joinedAt / 1000)}:R>
	  ...
	  `});
		if (client.guilds.cache.get(ramalcim.GuildID).members.cache.get(interaction.targetId).permissions.has(PermissionsBitField.Flags.Administrator) || Ayarlar.teyitciRolleri.some(x => client.guilds.cache.get(ramalcim.GuildID).members.cache.get(interaction.targetId).roles.cache.has(x))) 
	  ...
	  `• Toplam kayıt: ${registerData ? registerData.top : 0} • Erkek kayıt : ${registerData ? registerData.erkek : 0} • Kadın kayıt : ${registerData ? registerData.kız : 0} •`})
		...
		collector.on("collect", async (menu) => {
		   if(menu.values[0] === "avatar") {
			let member = client.guilds.cache.get(ramalcim.GuildID).members.cache.get(interaction.targetId);
