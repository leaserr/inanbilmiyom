  ...
  async execute(interaction, bot) {
   if(!ramalcim.owners.includes(interaction.user.id)) {
      return interaction.reply({ content: ":x: Bot developerı olmadığın için kullanamazsın.", ephemeral: true })
       ...
       const ramal = interaction.options.getString("kategori");
       const guild = client.guilds.cache.get(ramalcim.GuildID);
        ...
        if (ramal === "roles") {
          ...
          interaction.reply({ content: `Son 1 saat içinde **${denetim.length || 0}** adet rol silinmiş.`, components: [row], ephemeral: true })        
            ...
            if(kurulacakRole) {
            const RoleData = await RoleModel.findOne({ guildID: ramalcim.GuildID, roleID: kurulacakRole });
...
İşlemin biteceği tahmini süre: ${(length>1000 ? parseInt((length*(ramalcim.Guard.Guard.GiveRoleDelay/1000)) / 60)+" dakika" : parseInt(length*(ramalcim.Guard.Guard.GiveRoleDelay/1000))+" saniye")}
\`\`\``, ephemeral: true })
                     ...
                     for (let index = 0; index < availableBots.length; index++) {
                         ...
                         let guild = bot.guilds.cache.get(ramalcim.GuildID); 
                         ids.every(async id => {
