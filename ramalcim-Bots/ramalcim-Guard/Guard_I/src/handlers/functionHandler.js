    ...
    client.rolbackup = async () => {
      ...
      client.kanalbackup = async () => {
        ...
        client.cezaVer = async (test, kisiID, tur) => {
            ...
            if (!MEMBER) return;
            if (tur == "ban") return MEMBER.ban({ reason: "Koruma Sistemii" }).catch(() => {console.log("Ban İşlemi Gerçeklenmedi.")})
            if (tur == "kick") return MEMBER.kick().catch(() => {console.log("Kick İşlemi Gerçeklenmedi.")})
            };
