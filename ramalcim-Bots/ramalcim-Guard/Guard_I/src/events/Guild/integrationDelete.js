...
.setDescription(`
\`•\` Uygulanan İşlem: **[İntegration-Create]**
\`•\` İşlem Uygulayan: ${entry.executor} (\`${entry.executor.id}\`)
\`•\` İşlem Detayı: **${entry.executor} adlı üye bir integration kaldırdı.**
\`•\` Yapılan İşlem: **Kişi sunucudan atıldı.**
`)
newRole.guild.channels.cache.find(x => x.name == "protection_log").send({ embeds: [Embed2] });
};
