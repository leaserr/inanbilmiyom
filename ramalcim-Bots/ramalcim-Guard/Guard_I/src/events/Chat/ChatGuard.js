...
let küfürler = ["ramalcimoc","ramalcimoç","ramalcimamk","ramalcimaq","0r0spuc0cu","4n4n1 sk3r1m","p1c","@n@nı skrm","evladi","orsb","orsbcogu","amnskm","anaskm","oc","abaza","abazan","ag",
"a\u011fz\u0131na s\u0131\u00e7ay\u0131m","fuck","shit","ahmak","seks","sex","ramalcims\u0131z","amar\u0131m","ambiti","am biti","amc\u0131\u011f\u0131","amc\u0131\u011f\u0131n",
...
module.exports = async (message) => {
    ...
    if (ramalcim.Guard.kufurEngel && küfürler.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(message.content))) return sendChat(message, "Güzel kardeşim küfür senin o güzel diline hiç yakışıyormu ayıp.")
    ...
    if ((ramalcim.Guard.capsEngel) && (Caps / message.content.length) >= 0.7) return sendChat(message, "Caps-Lock kullanmayı lütfen azaltalım.")

    if (ramalcim.Guard.reklamEngel && message.content.match(inviteEngel)) {
    ...
    if(ramalcim.Guard.reklamEngel && reklamlar.some(word => message.content.toLowerCase().includes(word))) return sendChat(message, "Lütfen reklam yapmayı bırakınız.")
}
...
client.on("messageCreate", async (message) => {
  if(message.webhookID || message.author.bot || message.channel.type === ChannelType.DM) return;
  if (await client.checkPermission(client, message.author.id, "full") || await client.checkPermission(client, message.author.id, "chatguard")) return;
  if (ramalcim.Guard.spamEngel == false) return;
...
client.on('messageUpdate', async (oldMessage, newMessage) => {
    ...
    if (ramalcim.Guard.kufurEngel && küfürler.some(word => new RegExp("(\\b)+(" + word + ")+(\\b)", "gui").test(newMessage.content))) newMessage.delete().catch(err => {});
    ...
    if (ramalcim.Guard.reklamEngel && newMessage.content.match(inviteEngel)) {
    ...
    if(ramalcim.Guard.kufurEngel && reklamlar.some(word => newMessage.content.toLowerCase().includes(word))) return newMessage.delete().catch(err => {})
});
