...
module.exports = function (client) {
    ...
    if (ramalcim.Guard.Token.TaçHesapToken.length > 0) {
        tacuserX.on('ready', async () => {
          console.log(`${tacuserX.user.username} isimli Taç Guard Aktif!`);
        });
        tacuserX.login(ramalcim.Guard.Token.TaçHesapToken)  
    }

  client.checkPermission = async (bots, id, type) => {
    let member = client.guilds.cache.get(ramalcim.GuildID).members.cache.get(id);
    let res = await SafeMember.findOne({
        guildID: ramalcim.GuildID
    });
    ...
    if (!res) {
        ...
        if (ramalcim.owners.some(uye => uye == member?member.id:false) || res.Full.some(uye => uye == member?member.id:false  || member ? member.roles.cache.has(uye) : false)) {
            return true;
        }
        if (type == "full") {
  ...
  client.cezaVer = async (test, kisiID, tur) => {
    let MEMBER = test.guilds.cache.get(ramalcim.GuildID).members.cache.get(kisiID)
    if (!MEMBER) return;
    if (tur == "jail") return MEMBER.roles.cache.has(setup.boosterRolu) ? MEMBER.roles.set([setup.boosterRolu, setup.jailRole[0]]) : MEMBER.roles.set(setup.jailRole).catch(() => {if (ramalcim.Guard.Token.TaçHesapToken.length > 0) {client.taccezaVer(MEMBER.user.id, "tjail")}})
    if (tur == "ban") return MEMBER.ban({ reason: "Guard Sistem Koruma" }).catch(() => {if (ramalcim.Guard.Token.TaçHesapToken.length > 0) {client.taccezaVer(MEMBER.user.id, "tban")}})
    if (tur == "kick") return MEMBER.kick().catch(() => {if (ramalcim.Guard.Token.TaçHesapToken.length > 0) {client.taccezaVer(MEMBER.user.id, "tkick")}})
    };
    
  client.taccezaVer = async (kisiID, tur) => {
    tacuserX.guilds.cache.get(ramalcim.GuildID).members.fetch().then(async (x) => {
    let MEMBER = tacuserX.guilds.cache.get(ramalcim.GuildID).members.cache.get(kisiID)
    if (!MEMBER) return;
