let { ramal } = require('./Voices.Global.Client');
const { ActivityType } = require('discord.js');
const { joinVoiceChannel, getVoiceConnection } = require('@discordjs/voice');
const ramalcim = require('./../../Global/BotSettings/Settings');
require("./src/handlers/mongoHandler");

for (let index = 0; index < ramalcim.Welcome.Tokens.length; index++) {
    let token = ramalcim.Welcome.Tokens[index]
    let channel = ramalcim.Welcome.Channels < 1 ? ramalcim.Welcome.Channels[0] : ramalcim.Welcome.Channels[index]
    if(channel) {
        let client = new ramal()
            ...
            cron.schedule('* * * * 5', () => {
                ...
                if (day === 5) {
                    client.guilds.cache.forEach(async (guild) => {
                        ...
                        await Database.findOneAndUpdate({ guildID: ramalcim.GuildID }, {SesMod: xd}, { upsert: true })
                    ...
                    client.guilds.cache.forEach(async (guild) => {
                        ...
                        await Database.findOneAndUpdate({ guildID: ramalcim.GuildID }, {SesMod: xd2}, { upsert: true })
